...
# 5.1 计算概率（不影响你后面的生成输出）
safety_probs  = compute_safety_probs(model, tokenizer, messages)
refusal_probs = compute_refusal_probs(model, tokenizer, messages)

print("=== Safety probs (from next-token logits) ===")
for k, v in safety_probs.items():
    print(f"{k:15s}: {v:.4f}")
print("\n=== Refusal probs (from next-token logits) ===")
for k, v in refusal_probs.items():
    print(f"{k:15s}: {v:.4f}")

# 5.2 如需保持你原本的“完整生成输出”：
model_inputs = tokenizer([tokenizer.apply_chat_template(messages, tokenize=False)],
                         return_tensors="pt").to(model.get_input_embeddings().weight.device)
gen = model.generate(**model_inputs, max_new_tokens=128)
new_tokens = gen[0][model_inputs["input_ids"].shape[1]:]
content = tokenizer.decode(new_tokens, skip_special_tokens=True)
print("\n=== Generated content (unchanged format) ===")
print(content)
