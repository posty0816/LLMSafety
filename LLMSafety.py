...
'''
def model_inference(prompt: str, response: str = None, strict: bool = False):
    ...
    # poly_prompt_score, poly_response_score = infer_scores(model_poly, prompt, response)
...
if __name__ == "__main__":
    ...
    model_inference(prompt=prompt, response=response, strict=False)
...
# # 1) 确认加载到的模块与路径
# m = importlib.import_module("sel.Qwen3Guard_Score")
# print("Loaded module file:", m.__file__)

# # 2) 看看模块里有哪些符号跟 infer 相关
# print("Names containing 'infer':", [n for n in dir(m) if "infer" in n.lower()])

# # 3) 明确是否存在 inference
# print("Has 'inference'?:", hasattr(m, "inference"))
