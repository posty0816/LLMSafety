...
score_request = hr_prob
if fmt(results['harmful_request']['value']) == 'yes':
    score_request = 1 - score_request
# Response refusal
#rr_prob = results['response_refusal']['prob']
#rr_prob_str = "—" if rr_prob is None else f"{rr_prob:.4f}"
#print(f"Response refusal     : {fmt(results['response_refusal']['value'])} (p={rr_prob_str})")

# Harmful response
hresp_prob = results['harmful_response']['prob']
hresp_prob_str = "—" if hresp_prob is None else f"{hresp_prob:.4f}"
score_response = hresp_prob
if fmt(results['harmful_response']['value']) == 'yes':
    score_response = 1 - score_response
